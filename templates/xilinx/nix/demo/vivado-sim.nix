...
let
  ...
  view-waves = writeShellScriptBin "view-waves" ''
    ...
    echo "Opening FSDB: $FSDB_ABS"

    if [ -n "$DAI_PATH" ]; then
      DAI_ABS=$(readlink -f "$DAI_PATH")
      echo "Using Design Database: $DAI_ABS"
